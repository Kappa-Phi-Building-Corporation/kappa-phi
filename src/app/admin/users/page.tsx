...
function UserRow({ row, memberMap, allMembers, currentUserId }: {
  row: Row
    ...
    <tr className="border-b border-kp-border hover:bg-kp-card/50 transition-colors">
      ...
      <td className="px-4 py-3">
        <div className="flex flex-wrap gap-2 items-center">
          ...
          {p?.is_approved && p.role === 'member' && row.id !== currentUserId && (
          ...
          {p?.is_approved && p.role === 'admin' && row.id !== currentUserId && (
          ...
          {p?.is_approved && p.role === 'website_admin' && row.id !== currentUserId && (
            <form action={setRole.bind(null, row.id, 'member')}>
...
function Section({ title, items, accent, memberMap, allMembers, currentUserId }: {
  title: string
  items: Row[]
  accent?: string
  memberMap: Map<string, MemberOption>
    ...
    <div className="bg-kp-surface border border-kp-border rounded-2xl overflow-hidden">
      ...
      <div className="overflow-x-auto">
        <table className="w-full text-sm">
          ...
          <tbody>
            {items.map(row => (
              <UserRow key={row.id} row={row} memberMap={memberMap} allMembers={allMembers} currentUserId={currentUserId} />
            ))}
          </tbody>
        </table>
      </div>
...
export default async function AdminUsersPage() {
  ...
  const rows: Row[] = authUsers.map(au => {
  ...
  return (
    <div className="bg-kp-dark min-h-screen">
      ...
      <div className="max-w-6xl mx-auto px-4 py-10 space-y-8">
        ...
        <Section title="Pending Approval" items={pending} accent="bg-yellow-900/40" memberMap={memberMap} allMembers={allMembers} currentUserId={currentUser.id} />
        <Section title="Approved Users" items={approved} memberMap={memberMap} allMembers={allMembers} currentUserId={currentUser.id} />
        <Section title="Awaiting Email Confirmation" items={unconfirmed} accent="bg-gray-800" memberMap={memberMap} allMembers={allMembers} currentUserId={currentUser.id} />
        <div className="flex gap-6 pt-2">
          <Link href="/admin/link-requests"
